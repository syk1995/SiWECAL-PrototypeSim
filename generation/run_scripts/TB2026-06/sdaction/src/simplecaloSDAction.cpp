...
bool Geant4SensitiveAction<simplecaloSDData>::process(
  ...
  */
  ...
  // if (stepLength > 0.3){
  //   std::cout << "Warning: large step length in simplecaloSDAction: "
  //             << stepLength << " mm" << std::endl;
  // }
  G4double steplength = aStep->GetStepLength();
  G4double charge = aStep->GetTrack()->GetParticleDefinition()->GetPDGCharge();
  // if (steplength >0.5 && charge !=0) {
  if (steplength >0.5) {
    std::cout<<"Warning: large step length in simplecalo: "<<steplength<<" mm for particle with charge " 
    << charge << " and pdg code " 
    << aStep->GetTrack()->GetParticleDefinition()->GetPDGEncoding() << " and name " 
    << aStep->GetTrack()->GetParticleDefinition()->GetParticleName() << std::endl;
  }      
